...
import { LightningBoltIcon } from '../ui/Icons.tsx';
...
const IntenseUrgeButton: React.FC<IntenseUrgeButtonProps> = ({ user, userProfile, setUserProfile }) => {
    ...
    const [urgeContent, setUrgeContent] = useState<string[]>([]);

    useEffect(() => {
        const q = query(collection(db, "urge_content"), orderBy("createdAt", "asc"));
        const unsubscribe = onSnapshot(q, (snapshot) => {
            if (!snapshot.empty) {
                const content = snapshot.docs.map(doc => doc.data().text as string);
                setUrgeContent(content);
            } else {
                setUrgeContent([]);
            }
        });
        ...
        <>
            ...
            >
                <div className="text-right">
                    <h3 className="text-lg font-bold text-purple-300">رغبة شديدة</h3>
                    <p className="text-sm text-sky-400">اعطني حلاً سريعاً</p>
                </div>
